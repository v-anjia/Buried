...
class ADB_SN(object):
    ...
    def check_adb_device_isalive(self):
        '''
        function:get sn serial number
        :return:
        '''
        ...
        while True:
            ...
            p = re.compile("\n(.*)\t")
            if len(re.findall(p, ret.decode())) == 0:
                serial = Serial()
                print(len(re.findall(p, ret.decode())))
                serial.open_adb_through_serial(self.count)
                time.sleep(5)
            else:
                return re.findall(p, ret.decode())[0]
...
class Serial(object):
    ...
    def set_serialport(self):
        self.serial_port = self.package.update_fota_package()[1]
    ...
    def open_adb_through_serial(self,count):
        ...
        ser.write("am start -a home -n com.android.settings/com.android.settings.livingui.LVSettingHomePageActivity\r\n".encode('utf-8'))
        time.sleep(5)
...
def removal(stg):
    ...
    :return:
    '''
    new_stg = str(stg, encoding="utf-8").replace('\r\n','')
    return new_stg
